...
from core.common.consts import EXCEPT_PATH_LIST, EXCEPT_PATH_REGEX
from core.database.conn import db
from core.database.schema import Users, ApiKeys
from core.errors import exceptions as ex

from core.common import config, consts
from core.errors.exceptions import APIException, SqlFailureEx, APIQueryStringEx
...
async def access_control(request: Request, call_next):
    ...
    try:
        if url.startswith("/image_ai/analyzer_save"):
            ...
            if not config.conf().DEBUG:
                ...
                if "authorization" in headers.keys():
                    ...
                    if key and key.startswith("Bearer "):
                        ...
                        key = key
                    payload = jwt.decode(key, dotenv_values(base_dir + '/.env').get("JWT_SECRET"), algorithms=["HS256"])
